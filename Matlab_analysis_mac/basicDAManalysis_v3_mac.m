function basicDAManalysis_v3_mac()
close all; clear all;
% Copy and paste your directory into the single quotation marks below.
%rootdir = '/Volumes/Camilo_SSD/Sleep_runs/VNC_project/AN_PPK/20260803';
rootdir = '/Volumes/Camilo_SSD/Sleep_runs/reversibility/20260804';
plotAreaQuartiles = 0;
...
groupNames = {'23E10GS_pex16_male';	'23E10GS_pex16_female';	'pex16_control_male';	'pex16_control_female';	'23E10GS_control_male';	'23E10GS_control_female'};
%Should be as many rows of three column inputs as there are groupNames.
groupColors = rand(size(groupNames,1),3);

% ===== ADDED: prepare list of temporary page PDFs =====
pagePDFs = {};  % will collect per-figure temporary pdf filenames
    ...
    for(ci = 1:size(groupNameByChannel,1)),
        if(strcmp(groupNames{gi},groupNameByChannel{ci})),
            ...
            if(~isempty(thisChannelDatByDay)),
                ...
                % ===== CHANGED PRINTING: write one temporary per-page PDF =====
                pageName = sprintf('%s_page_%03d.pdf', output, numel(pagePDFs)+1);
                print(figure(1), '-dpdf', pageName);
                pagePDFs{end+1} = pageName;
...
% ===== Merge the temporary page PDFs into one final PDF =====
if ~isempty(pagePDFs)
    finalPDF = [output '.pdf'];
    try
        mergePDFs(finalPDF, pagePDFs);
        % delete temp pages if merge succeeded
        for i = 1:numel(pagePDFs)
            if exist(pagePDFs{i},'file')
                delete(pagePDFs{i});
            end
        end
        disp(['Final PDF saved as: ' finalPDF]);
    catch ME
        warning('Could not merge PDFs automatically. Temporary PDFs left in folder. Error: %s', ME.message);
        disp('Temporary page PDFs:');
        disp(pagePDFs');
    end
else
    disp('No pages were generated; no PDF to merge.');
end
end
% end;
% figure(2);
...
% ===== Helper: mergePDFs =====
function mergePDFs(outputFile, pdfList)
% mergePDFs tries multiple strategies:
% 1) use MATLAB's append_pdfs (if available)
% 2) fallback to Ghostscript (system call)
% If neither is available, error is thrown.

% Basic checks
if isempty(pdfList)
    error('mergePDFs:NoInput','No input PDFs provided.');
end

% Ensure files exist
for k = 1:numel(pdfList)
    if ~exist(pdfList{k},'file')
        error('mergePDFs:MissingFile','File does not exist: %s', pdfList{k});
    end
end

% Strategy 1: try append_pdfs (MATLAB File Exchange / bundled in some releases)
if exist('append_pdfs','file') == 2
    try
        append_pdfs(outputFile, pdfList{:});
        return;
    catch ME
        warning('mergePDFs:append_failed','append_pdfs failed: %s', ME.message);
    end
end

% Strategy 2: try Ghostscript (gs) system call
% Build gs command for pdfwrite
cmd = ['gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile="' outputFile '"'];
for k = 1:numel(pdfList)
    cmd = [cmd ' "' pdfList{k} '"'];
end
...
if status == 0
    ...
    error('mergePDFs:NoMergeMethod','Could not merge PDFs automatically. Try installing Ghostscript or append_pdfs. System message: %s', cmdout);
end
